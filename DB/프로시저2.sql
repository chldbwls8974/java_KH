...
BEGIN 
    ...
    */
    ...
    SET _RV_NUM = CONCAT(
        ...
        'MS',
        LPAD(_SD_NUM, 3, 0),
        LPAD((SELECT COUNT(*)+1 FROM RESERVATION WHERE RV_SD_NUM = _SD_NUM) , 3, 0)
	);
    ...
    IF _IS_DISCOUNT = 'Y' THEN 
        ...
        (SELECT PR_DC_PRICE FROM PRICE WHERE PR_TYPE='청소년')*_CHILD_COUNT;
	ELSE
		SET _ADULT_TOTAL_PRICE = 
        (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE='성인')*_ADULT_COUNT;
        SET _CHILD_TOTAL_PRICE = 
        (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE='청소년')*_CHILD_COUNT;
    END IF;
    -- 전체 요금을 계산 
    SET _TOTAL_PRICE = _ADULT_TOTAL_PRICE + _CHILD_TOTAL_PRICE;
	-- 예약 정보를 테이블에 추가 
    INSERT INTO RESERVATION(RV_NUM, RV_SD_NUM, RV_ME_ID, RV_ADULT, RV_CHILD, RV_PRICE)
    VALUE(_RV_NUM, _SD_NUM, _ID, _ADULT_COUNT, _CHILD_COUNT, _TOTAL_PRICE);
